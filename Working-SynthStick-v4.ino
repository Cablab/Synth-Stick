...
// Used to make music!
int instruments[] = {2, 3, 4, 7, 9, 11}; // Holds desired instruments
int instIndex = 0;
...
void loop() {    
  ...
  boolean betweenFrets = ((softPotADC - lastPotADC) <= 5) && ((softPotADC - lastPotADC) >= -5);

  talkMIDI(0xC0, instrument, 0); //Set instrument number. 0xC0 is a 1 data byte command
  
  //if pot is pressed, play note
  if (softPotADC > 10 && newNote && !betweenFrets) {
...
void instrumentUp() {
  if (instIndex == ((sizeof(instruments) / sizeof(instruments[0])) - 1)) {
      instIndex = 0;
...
void instrumentDown() {
  if (instIndex == 0) {
    int lastInst = ((sizeof(instruments) / sizeof(instruments[0])) - 1);
      instrument = instruments[lastInst];
  } 
  else {
      instrument = instruments[--instIndex];
  }
